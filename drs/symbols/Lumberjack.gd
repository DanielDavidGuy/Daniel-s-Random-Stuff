...
func add_conditional_effects(symbol, adjacent):
	for i in adjacent:
		symbol.add_effect_for_symbol(i, effect().if_group("lumber").set_destroyed().animate("bounce", 0, [symbol, i]))
		symbol.add_effect_for_symbol(i, effect().if_destroyed().if_group("lumber").set_target(symbol).add_permanent_bonus(values[0]))
